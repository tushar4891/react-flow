...
function ReactRendererFlow() {
    ...
    if (nodeId !== "0") {
      ...
      for (; i < nodes.length; i++) {
        if (nodes[i].id === nodeId) {
          nodes[i].data.strings.number = text.number;
          nodes[i].data.strings.type.label = text.type.label;
        }
      }
    } else {
      const newNodeId = `${nodes.length + 1}`;
  ...
  const reColdNode = (text) => {
    if (nodeId !== "0") {
      ...
      for (; i < nodes.length; i++) {
        if (nodes[i].id === nodeId) {
          nodes[i].data.strings.template.label = text.template.label;
          nodes[i].data.strings.emailOptions.label = text.emailOptions.label;
        }
      }
    } else {
      const newNodeId = `${nodes.length + 1}`;
  ...
  const handleSave = async () => {
    const graphData = {
      userId: "user123", // Replace with a unique identifier for the user
      nodes,
      edges,
    };

    try {
      const response = await fetch("http://localhost:4000/api/saveGraph", {
        method: "POST",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh" }}>
      ...
      >
        <button
          style={{
            backgroundColor: "#015afe",
            color: "white",
            border: "2px solid #b4c5ef",
            borderRadius: "5px",
          }}
          onClick={handleSave}
        >
          <IoRocketOutline /> Save & Schedule
        </button>
      </div>
      {/* React Flow Chart */}
      <div style={{ flex: 1 }}>
              ...
              node.id === "3"
                ...
                : {
                    ...
                    data: {
                      ...
                      onNodeClick: () => handleNodeClick(node.id), // just added
                    },
                  }
            )}
